...
def delete():
    ...
    x = messagebox.askyesno("Password Keeper !ALERT!", "Are you Sure?")
    if x == True:
...
def submit(pas, opas):
    ...
    if list(abc)[0] != opas:
        ...
        admin_pass_old.delete(0, END)
        admin_pass.delete(0, END)

        btn = Button(login_frame, text="Click to Delete all Data", command=delete)
...
admin_pass_old_label = Label(login_frame, text="Current Admin Password :", font="Arial 15")
admin_pass_old_label.grid(row=0, column=0)

admin_pass_old = Entry(login_frame, width=20)
admin_pass_old.grid(row=0, column=1)

admin_pass_old.insert(0, "password")

admin_pass_label = Label(login_frame, text="New Admin Password :", font="Arial 15")
admin_pass_label.grid(row=1, column=0)

admin_pass = Entry(login_frame, width=20)
admin_pass.grid(row=1, column=1)

submit_btn = Button(login_frame, text="Submit", font="Arial 15",
                    command=lambda: submit(admin_pass.get(), admin_pass_old.get()))
